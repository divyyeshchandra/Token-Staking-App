...
function genrateCountDown() {
  ...
  var mintutes = Math.floor((now % (1000 * 60 * 60)) / (1000 * 60));
  var seconds = Math.floor((now % (1000 * 60)) / 1000);
  return mintutes + "m" + seconds + "s";
}
...
const userTsansactionHistory = userTransaction.map((transaction, i) => {
  return `
        <div
...
const userProfileHTML = `
    <div class="contract-user-profile">
        ...
        <span class="contract-space"
            ><strong>Last Reward Calculation Time:&nbsp;</strong>${genrateCountDown(
              user.lastRewardCalculationTime
            )}</span
        >
        <span class="contract-space"
            ><strong>Last Stake Time:&nbsp;</strong>${genrateCountDown(
              user.lastStakeTime
            )}</span
        >
        <span class="contract-space"
        ...
        <p class="contract-paragraph">
            Lorem ipsum dolor sit amet consrcyvh bufbjbjfv elit. harum
            bfbv vhfbvbfybf bvfbvbfj uvbfbvufbbvufubv fubvbf
        </p>
        </div>
    </div>
`;
...
contractTransactionList.innerHTML = userTsansactionHistory;
